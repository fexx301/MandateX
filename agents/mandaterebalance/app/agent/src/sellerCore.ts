...
import {
  quoteRejectionEnvelope,
  RebalanceDeliveryApi,
  RebalanceService,
} from "./rebalance.js";
...
export interface SellerCoreOpts {
  ...
  /** Test seam and domain hook for the deterministic MandateX service. */
  rebalance?: RebalanceDeliveryApi;
}
...
export class SellerCore {
  ...
  protected readonly rebalance: RebalanceDeliveryApi;
  ...
  constructor(opts: SellerCoreOpts) {
    ...
    this.rebalance =
      opts.rebalance ?? new RebalanceService({ network: this.network });
  ...
  async negotiate(
    ...
    const prepared = await this.rebalance.prepareQuote(request);
    if (!prepared.ok) return quoteRejectionEnvelope(prepared.refusal);
    const clamped = this.signing.clampPrice(this.signing.listPrice());
    const signed = await this.signing.signQuote(prepared.request, clamped);
    return {
      ...signed,
      // This is a display convenience only. The authoritative mandate,
      // evidence, proposal, and permissions are inside the signed task text.
      mandatex: {
        schema: prepared.signedTask.schema,
        mandate_id: prepared.signedTask.mandate.mandate_id,
        observed_block: prepared.signedTask.evidence.observed_block,
        proposed_lower_tick: prepared.signedTask.proposal.proposed_lower_tick,
        proposed_upper_tick: prepared.signedTask.proposal.proposed_upper_tick,
        display_only: true,
      },
    };
  }
  ...
  protected async doWorkAndSubmit(
    ...
    if (spec !== null) {
      const deterministicReceipt = await this.rebalance.deliver(jobId, {
        task: spec.task,
        terms: spec.terms,
      });
      if (deterministicReceipt !== null) {
        let res: { submitTx: string; deliverableUrl: string | null };
        try {
          res = await this.signing.submitResult(
            jobId,
            JSON.stringify(deterministicReceipt),
            {
              ...
              mandatex_receipt_status: deterministicReceipt.status,
            },
          );
        } catch (e) {
          if (
        ...
        return {
          ok: true,
          job_id: jobId,
          outcome: deterministicReceipt.status,
...
export function parseJobId(raw: unknown): number {
  if (typeof raw === "number") {
    ...
    throw new Error("job_id must be a non-negative safe integer");
  }
  let parsed: bigint;
  try {
  ...
  if (parsed < 0n || parsed > BigInt(Number.MAX_SAFE_INTEGER)) {
    throw new Error("job_id exceeds the safe integer range");
  }
  return Number(parsed);
}

function extractRequest(data: Record<string, unknown>): Record<string, unknown> {
  const candidate = data.request;
  if (
    candidate !== null &&
    ...
    if (request.mandate === undefined && data.mandate !== undefined) {
      request.mandate = data.mandate;
    }
    return request;
  }
  return {
    ...(data.task_description !== undefined
      ? { task_description: data.task_description }
      : {}),
    ...(data.terms !== undefined ? { terms: data.terms } : {}),
    ...(data.mandate !== undefined ? { mandate: data.mandate } : {}),
  };
}
